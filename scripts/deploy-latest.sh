...
case "${ENVIRONMENT}" in
  ...
  *)
    ...
    echo "Usage: scripts/deploy-latest.sh [dev|staging|prod]"
    exit 1
    ;;
esac
...
IMAGE_TAG="$(first_non_empty "${IMAGE_TAG:-}" "latest" || true)"
...
ALSO_TAG_VERSION="$(first_non_empty "${ALSO_TAG_VERSION:-}" "true" || true)"
...
VERSION_REF="${REGISTRY}/${IMAGE_REPOSITORY}:${VERSION}"
...
if [[ "${ALSO_TAG_VERSION}" == "true" ]]; then
  echo "Additional version tag: ${VERSION_REF}"
fi
